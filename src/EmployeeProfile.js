  ...
  const EmployeeProfile = () => {
    ...
    useEffect(() => {
      const fetchEmployee = async () => {
        ...
        try {
          ...
          setLoading(false);
          setFaceAdded(response.data.faceAdded);
        } catch (error) {
    ...
    const handleSaveChanges = async () => {
      try {
        ...
        const currentEmployeeResponse = await API.get(`/admin/user/${id}`);
        ...
        // Preserve faceAdded status if it's true
        if (currentEmployee.faceAdded) {
          employeeData.faceAdded = true;
        }
    
        // Proceed with the update
        await API.post(`/admin/user/${id}`, employeeData);
        setDialogOpen(true);
        setIsEditMode(false);
      } catch (error) {
